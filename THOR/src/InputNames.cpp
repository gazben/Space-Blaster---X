...
namespace
{
	...
	class StringBimap
	{
		THOR_STATIC_ASSERT(( !std::tr1::is_same<T, std::string>::value ));
	...
	StringBimap<sf::Keyboard::Key> InitMap(detail::Type2Type<sf::Keyboard::Key>)
	{
		...
		THOR_STATIC_ASSERT(sf::Keyboard::KeyCount == 101);
		#undef THOR_INSERT_MAPPING

		return bimap;
	}

	StringBimap<sf::Mouse::Button> InitMap(detail::Type2Type<sf::Mouse::Button>)
	{
		...
		THOR_STATIC_ASSERT(sf::Mouse::ButtonCount == 5);
		#undef THOR_INSERT_MAPPING
	...
	{
		static StringBimap<T> instance = InitMap(detail::Type2Type<T>());
		return instance;
	}
